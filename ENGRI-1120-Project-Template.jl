...
html"""
<p style="font-size:20px;">ChemE Team (pronounced "Cheem," for the sake of artistry and style): Gavin Aksoy, Pedro Moreira, Gavi Schechter, Joe Schillinger</br>
...
# ╔═╡ 5bb6255e-f993-49e5-aed5-d243901b5191
html"""
<p style="font-size:20px;">For our project, we were tasked with developing a process to convert Xylan (a polymer for xylose, a five-carbon sugar molecule, derived from wood. For the purpose of this project, the sponsor has asked that we approximate Xylan as Xylose) into 1,2-Propanediol dinitrate, also known as PGDN (a small molecule used in propellants and explosives). The task is to have a 1g/hr flow rate in our downstream, with PGDN being 95% of the downstream (PGDN needs to be 95% pure). In addition to producing the compound of interest, we must analyze the financial performance of our process to determine its practical feasibility by comparing it to an alternative investment yielding a minimum of 1% per year and a maximum of 10% per year (constant) using a Net Present Value (NPV) approach.
"""
...
# ╔═╡ 52ff9213-cf65-48ce-958f-73b87d9b75c6
number_of_chips = 6

# ╔═╡ 47b1c30a-158c-44cc-9fd1-87a9eb6dc0fe
begin
	# alias the mass flow into the sep-units
	mass_flow_into_seps = (number_of_chips) * mass_dot_output_array
...
# ╔═╡ d4206768-d25c-42b1-ad9e-6c05923650d8
md"""
### NPV
"""

# ╔═╡ 3d336de4-cb1e-47c6-84d7-f4ca2cfd6beb
md"""
##### Sheet with financial info: https://docs.google.com/spreadsheets/d/1xSMoBYjOx_HcUmavWj1Bt03Q7XudXtPoMcvhWvAxU58/edit?usp=sharing
"""

# ╔═╡ d1871ac0-9ae5-4cf0-abf1-8a0249ccd0dc
#For 4 years, 4 periods (per year)
T = 4

# ╔═╡ 473648d3-9fe9-452f-9372-aa633427595f
#Discount rate of 1% minimum
discount_rate_min= 0.01

# ╔═╡ f514f40b-acc8-42b4-97b1-cf14c6e0db40
#Discount rate of 10% maxiumum
discount_rate_max = 0.1

# ╔═╡ dd13660d-9eae-4745-bb01-4be553306d31
# Setup the cash flow array, values from Google Sheets -
# Capital costs: -$2.68K
# Operation Costs: -$3.07K 
# Year one costs: -$5.75K 
# Value of PGDN produced as combo of 1,2PD, KNO3: $2.25174K 
# Net cashflow per year: -$0.819K 
CF_array = [-5.75, -0.819, -0.819, -0.819, -0.819]

# ╔═╡ ecc07829-13b8-42a2-8c59-77da32e4145c
# compute the discount terms -
begin 
	# discount term for minimum 1% -
	discount_array_min = ones(T+1)
	for time_index = 2:(T+1)
    	tmp_term = (1+discount_rate_min)^(time_index-1)
    	discount_array_min[time_index] = tmp_term
	end
	# return -
	nothing
	
	# discount term for minimum 10% -
	discount_array_max = ones(T+1)
	for time_index = 2:(T+1)
    	tmp_term = (1+discount_rate_max)^(time_index-1)
    	discount_array_max[time_index] = tmp_term
	end
	# return -
	nothing
end

# ╔═╡ 0742eb94-00af-4450-b1e0-31cc96142ecd
# PV for minimum 1%
PV_min = CF_array.*(1.0./discount_array_min)

# ╔═╡ 735e5b7f-14e0-4499-a80e-46331fd43110
# PV for maximum 10%
PV_max = CF_array.*(1.0./discount_array_max)

# ╔═╡ 9e8349af-3ff6-4bd1-9768-d1312eb84659
# NPV for minimum 1%
NPV_min = sum(PV_min)

# ╔═╡ fee6e549-9cea-439c-84ee-5527502af3e9
# NPV for maximum 10%
NPV_max = sum(PV_max)
...
# ╔═╡ 3a0848da-0376-4028-9a79-5ecfe1f0ab70
md"""
##### What went into the reactors?
"""

# ╔═╡ d4f70dae-c394-45c6-ad3b-38bee470a48d
html"""
<p style="font-size:20px;">
In order to produce our final product of PGDN, we analyzed several reaction pathways to determine what materials should be supplied in addition to the D-Xylose feedstock. The PGDN product pathway requires KNO3, which we chose to purchase and supply into a stream with Glucose and Oxygen from the air. We chose glucose because it is low cost and, as a 6-carbon sugar, easily used in biochemistry rather than the Xylose, a 5-carbon sugar. Using glucose to supplement xylose made both the chemical and financial aspect of the process more practical and efficient. Glucose, O2, and KNO3 were placed in one mixer and pumped into each reactor in one stream, while D-Xylose was fed with another stream. The mol flow rate of glucose must have been low to prevent the chemistry from prioritizing its consumption rather than consuming D-xylose.
"""

# ╔═╡ 4304b933-6ff2-4a95-b42f-cdc19e749582
md"""
##### Parallel vs. Series?
"""

# ╔═╡ 48149e83-ee9a-47f2-9bf8-e193ed5249ca
html"""
<p style="font-size:20px;">
For our process, we could have used either reactors in series or in parallel. “Reactors in series” means reactors sequentially connected together, with the output stream of one reactor as the input stream of the next reactor. We would add glucose into each reactor to keep the reaction going in order to produce more of our desired product (PGDN), just enough such that glucose does not become the main reactant over D-Xylose. “Reactors in Parallel” means that we pump all of our reactants into two input streams (Glucose and Potassium Nitrate in stream 1, and Xylose in stream 2, respectively). The upside to using reactors in series is that we would need less stream splitters, which cost money, because the reactors would more efficiently consume excess reactants. However, for our project we assumed that stream splitters are free, and so a parallel model had less financial cost. 
We used six chips (reactors) and thus six reactions occurred simultaneously. With two mixers, one for each stream, a splitter connects the two and then splits them into six streams for each reactor in parallel. Then, a second steam splitter would connect the six output streams from the reactors into one stream for separation. Since our reactors were in parallel, we used two stream splitters (one at the end and one at the beginning) to first divide the streams into 6, and then combine them into one stream for separation. With two streams, our 1st stream splitter, we used 2 syringe pumps to supply our reactor. 

"""

# ╔═╡ 3990dc44-54a1-4391-8f32-43f39ee14fd8
md"""
##### Separations
"""

# ╔═╡ ba66ffdf-25b0-4eed-bc48-2f94b35c46cb
html"""
<p style="font-size:20px;">
Throughout the process, 4 stages of separation were necessary to achieve a purity of 95% for the PGDN output. The purity of PGDN in our downstream after 4 separations was 95.04%. Such choice also contributed in reducing the cost of separators, which are further discussed in the following financial section. 

"""

# ╔═╡ 40e50796-983e-4781-ad82-c77b0b01c6de
md"""
##### Finances and Costs
"""

# ╔═╡ f1a8de4c-d51f-42b5-9105-4d9265797d6a
html"""
<p style="font-size:20px;">
All of the well-mixed reactors, universal separators, stream splitters, and syringe plungers were provided by VarnerLab. The cost of reactors was $100.00 per unit, the cost of syringe plungers was $1000.00 per pump, separators cost $20.00 each, and splitters were free of charge. In our process, 6 reactors, 4 separators, and 2 syringe pumps were utilized. This resulted in a cost of $2680.00 from machinery alone.

Concerning the cost of reactants, different compounds cost differently, according to Sigma Aldricht. A quantity of 0.5 g hr-1 of potassium nitrate was demanded by us. Hence, it resulted in a cost of 0.0288762 $ hr-1. Additionally, it utilized 0.90078 g hr-1 of glucose, which cost 0.229879056 $ hr-1. Lastly, it cost 0.09181470384 $ hr-1 to supply D-Xylose for our process. Per year, this is a supply cost of $3070.99
...
# ╔═╡ 080ba1f5-87e4-45fe-902d-f34836abc52b
html"""
<p style="font-size:20px;">
Assumptions made- All streams and chemicals are ideal, all reactors are isothermal (so no delving into the energy balances), we do no’t have to pay for waste disposal (including expensive disposal of water), stream splitters are free, we don’t have any workers to pay, and our process ran 24/7/365.
Because of all the assumptions listed above, our “ideal” process was cheaper and easier, than it would have been in real life. Even though our process was cheaper than it would have been in real life, our ideal process was still not economically viable:

"""

# ╔═╡ 1869c47d-d427-4a16-990c-f470e402c29a
html"""
<p style="font-size:20px;">
NPV_min:
-8.945709786857346 (1% discount rate) 
NPV_max:
 -8.34611980056007 (10% discount rate)
"""

# ╔═╡ d67a0dac-bf2f-4539-921c-3d4f14b48796
html"""
<p style="font-size:20px;">
In the future, there are different ways that we could make our process more practical in terms of profit. One of our products was Pyruvate, which we could potentially separate in “bottoms” in order to sell it. Selling pyruvate would generate more income for our process. We could also use carbon dioxide, another byproduct of our reaction, to a company that is dependent on using carbon dioxide for their products (Pepsi, Coca Cola,etc.). We could also recycle some of our unreacted starting material using series, or also produce KNO3 from the unused product KOH. 
...
# ╔═╡ c72ceb14-0186-4f62-9688-984ca12e0b10
html"""
<p style="font-size:20px;">
